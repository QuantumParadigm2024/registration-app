...
const FormPreview = ({ formData, onBack }) => {
    ...
    // Separate sections into top sections (header/logo) and bottom sections (sponsors/rules/description)
    const topSections = sortedSections.filter(section =>
        section.type === 'FORM_HEADER' || section.type === 'FORM_LOGO'
    );

    const bottomSections = sortedSections.filter(section =>
        section.type === 'FORM_SPONSORS' || section.type === 'FORM_RULES' || section.type === 'FORM_DESCRIPTION'
    );
        ...
        switch (section.type) {
            ...
            case 'FORM_SPONSORS':
                ...
                const groupSponsorsByTier = () => {
                    if (!sectionData.showTiers) return null;

                    const grouped = {};
                    sectionData.sponsors.forEach(sponsor => {
                ...
                return (
                    <div className="py-4 px-4 bg-gray-50">
                        ...
                        {sectionData.showTiers && groupedSponsors ? (
                            ...
                            renderSponsorsGrid(sectionData.sponsors)
                        )}
                    </div>
                );

            case 'FORM_RULES':
                if (sectionData.showSection === false || !sectionData.rules || sectionData.rules.length === 0) {
                    return null;
                }
                    ...
                    <div className="py-4 px-4">
                        <div className="max-w-2xl mx-auto">
                            <h2 className="text-base font-semibold mb-3">
                                {sectionData.title || 'Rules & Regulations'}
                            </h2>
                            <ul className="space-y-1 list-disc pl-6">
                                {sectionData.rules.map((rule, index) => (
            ...
            <div className="py-4 px-4">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                        {/* Render top sections (header and logo) */}
                        {topSections
                            .filter(section => {
                                const sectionData = section.dataJson ? JSON.parse(section.dataJson) : section.data || {};
                            ...
                            .map((section, index) => (
                                <div key={`top-${section.type}-${index}`}>
                        ...
                        {/* Render bottom sections (sponsors, rules, description) */}
                        {bottomSections
                            .filter(section => {
                                const sectionData = section.dataJson ? JSON.parse(section.dataJson) : section.data || {};
                                return sectionData.showSection !== false;
                            ...
                            .map((section, index) => (
                                <div key={`bottom-${section.type}-${index}`}>
                                    {renderSection(section)}
                                </div>
                            ))
                        }
                        ...
                        <div className="pt-3 flex flex-col items-center mb-4">
                            <button
