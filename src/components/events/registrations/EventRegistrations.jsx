...
const EventRegistrations = () => {
    ...
    const [savingConfig, setSavingConfig] = useState(false);
    const [loadingConfig, setLoadingConfig] = useState(false);
    const [loadingBadgeConfig, setLoadingBadgeConfig] = useState(false); // For initial badge config check
    ...
    // Fetch badge configuration ONLY when needed (not on mount)
    // We'll fetch it when opening the format modal or when generating badges
    ...
    // Fetch badge configuration only when needed (lazy load)
    const fetchBadgeConfig = async (showLoading = true) => {
        try {
    ...
    // Fetch form fields for configuration - only when opening modal
    const fetchFormFields = async () => {
        try {
            const response = await axiosInstance.get(`/events/${eventId}/form-fields`);
            if (Array.isArray(response.data)) {
    ...
    // Open format modal - fetch config and form fields only when opened
    const openFormatModal = async () => {
        // Show loading state
        setLoadingConfig(true);
        setShowFormatModal(true);
        
        // Fetch both config and form fields in parallel
        await Promise.all([
            fetchBadgeConfig(true),
            fetchFormFields()
        ]);
    ...
    // Fetch registrations - only this is needed on mount
    useEffect(() => {
        if (searchTimeout) {
            clearTimeout(searchTimeout);
        }
    ...
    // Remove the badge data fetch on mount - we'll fetch when needed
    ...
    // Fetch badge data only when generating badges
    const fetchBadgeData = async (forPreview = false) => {
        try {
            if (forPreview) setBadgeLoading(true);
            const response = await axiosInstance.get(
                `/events/${eventId}/badges/exportAll`
            );
            ...
            if (forPreview) setBadgeLoading(false);
        }
    };
    ...
    const handleCheckInToggle = async (registrationId, currentStatus) => {
        try {
            ...
            if (response.data?.status === 'success') {
                await fetchRegistrations();
                // Only fetch badge data if we're going to need it (optional)
                // await fetchBadgeData(false);
            } else {
                throw new Error('Failed to toggle check-in');
            }
        } catch (err) {
    ...
    const handlePreviewBadge = async (registrationId = null) => {
        // Fetch badge config first if not loaded
        if (!badgeConfig) {
            await fetchBadgeConfig(false);
        }
    ...
    const getBadgeCount = () => {
        // This is just a count from registrations, not from badgeData
                ...
                <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-6 w-full">
                    ...
                    <div className="mb-3 sm:mb-6">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            ...
                            {!isMobileView && (
                                <div className="flex items-center space-x-2 flex-shrink-0">
                                    {/* Format Badge Button - Now fetches config when clicked */}
                                    <button
                                        onClick={openFormatModal}
                                        className="px-3 py-1.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center text-sm"
                                    >
                    ...
                    <div className="min-h-screen px-2 sm:px-4 py-2 sm:py-8">
                        <div className="relative bg-white rounded-xl shadow-xl max-w-7xl mx-auto max-h-[98vh] sm:max-h-[90vh] overflow-y-auto">
                            <PreviewBadge
                                ...
                                allRegistrations={filteredRegistrations}
                                isSingleBadge={false}
                                badgeConfig={badgeConfig} // Pass config if needed
                            />
                        </div>
                    </div>
                </div>
